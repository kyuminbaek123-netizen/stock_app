...
def score_all(hist, info, patterns, macro, is_kr=False):
    ...
    # 5. PER (Trailing 우선 - 실제 가치)
    per = info.get('trailingPE') or info.get('forwardPE')
    if per and per > 0:
        if per < 12: s["PER"] = 85; reasons_p.append(f"PER {per:.1f} - 매우 저평가")
        elif per < 18: s["PER"] = 70; reasons_p.append(f"PER {per:.1f} - 저평가")
        elif per < 25: s["PER"] = 55
        elif per < 35: s["PER"] = 40
        elif per < 50: s["PER"] = 25; reasons_n.append(f"PER {per:.1f} - 고평가")
        else: s["PER"] = 10; reasons_n.append(f"PER {per:.1f} - 극단고평가")
    else:
        s["PER"] = 50
    ...
    if pbr and pbr > 0:
        if pbr < 1: s["PBR"] = 85; reasons_p.append(f"PBR {pbr:.2f} - 자산가치 저평가")
        elif pbr < 2: s["PBR"] = 70
        elif pbr < 4: s["PBR"] = 50
        elif pbr < 7: s["PBR"] = 30; reasons_n.append(f"PBR {pbr:.2f} - 고평가")
        else: s["PBR"] = 15; reasons_n.append(f"PBR {pbr:.2f} - 극단고평가")
    else:
        s["PBR"] = 50
    ...
    # 가중치 (종목 자체 75%, 매크로 25%) + 세력매집 비중 강화
    if is_kr:
        w = {"차트패턴": 0.13, "이평선": 0.10, "RSI": 0.05, "MACD": 0.05, "PER": 0.07,
             "PBR": 0.05, "성장성": 0.07, "일목": 0.07, "거래량": 0.05,
             "OBV": 0.08, "POC": 0.07, "VCP": 0.06,
             "VIX": 0.02, "공포탐욕": 0.03, "금리": 0.02,
             "유동성": 0.02, "신용": 0.01, "원화": 0.03}
    else:
        w = {"차트패턴": 0.13, "이평선": 0.11, "RSI": 0.05, "MACD": 0.05, "PER": 0.07,
             "PBR": 0.05, "성장성": 0.07, "일목": 0.07, "거래량": 0.06,
             "OBV": 0.09, "POC": 0.08, "VCP": 0.06,
             "VIX": 0.02, "공포탐욕": 0.03, "금리": 0.02,
             "유동성": 0.02, "신용": 0.00}
        tot = sum(w.values())
    ...
    if total >= 68: verdict, vclass = "적극 매수", "v-strong-buy"
    elif total >= 56: verdict, vclass = "매수", "v-buy"
    elif total >= 44: verdict, vclass = "중립 / 관망", "v-hold"
    elif total >= 32: verdict, vclass = "매도", "v-sell"
...
def backtest_prediction(hist, info):
    """6개월 전 시점에서 똑같이 예측 → 현재값과 비교"""
    if len(hist) < 252: return None
    # 6개월 전 시점
    past_idx = len(hist) - 126  # 6개월 ≈ 126영업일
    past_hist = hist.iloc[:past_idx].copy()
    if len(past_hist) < 100: return None

    # 과거 시점 데이터로 patterns + target 계산
    past_curr = past_hist['Close'].iloc[-1]
    past_high = past_hist['Close'].iloc[-min(252, len(past_hist)):].max()
    past_low = past_hist['Close'].iloc[-min(252, len(past_hist)):].min()
    ...
    # 방향 적중: 예측이 상승/하락이었나 vs 실제
    pred_dir = "상승" if past_target > past_curr else "하락"
    actual_dir = "상승" if actual_curr > past_curr else "하락"
    dir_match = pred_dir == actual_dir

    # 정확도 점수 (오차 작을수록 높음)
    if err_pct < 5: acc_score = 95
    elif err_pct < 10: acc_score = 80
    elif err_pct < 20: acc_score = 60
    elif err_pct < 30: acc_score = 40
    else: acc_score = 20
...
try:
    ...
    with h2:
        ...
        per_trl = info.get('trailingPE')
        # Trailing PER이 실제 가치 더 반영 → 우선 표시
        if per_trl and per_trl > 0:
            per_v = f"{per_trl:.1f}"
            if per_fwd and per_fwd > 0:
                per_sub = f"Trailing · Fwd {per_fwd:.1f}"
            else:
                per_sub = "Trailing"
        elif per_fwd and per_fwd > 0:
            per_v = f"{per_fwd:.1f}"
            per_sub = "Forward"
        else:
            per_v = "N/A"
            per_sub = "데이터 없음"
        st.markdown(card("PER", per_v, per_sub), unsafe_allow_html=True)
    with h3:
        pbr = info.get('priceToBook')
        if pbr and pbr > 0:
            pbr_v = f"{pbr:.2f}"
            ...
            pbr_v = "N/A"
            pbr_sub = "데이터 없음"
        st.markdown(card("PBR", pbr_v, pbr_sub), unsafe_allow_html=True)
    with h4:
    ...
    # 마일스톤 (첫 사진처럼 깔끔하게)
    ipo_price = float(hist['Close'].iloc[0])
    low_idx = hist['Close'].iloc[-252:].idxmin() if len(hist) >= 252 else hist['Close'].idxmin()
    high_idx = hist['Close'].iloc[-252:].idxmax() if len(hist) >= 252 else hist['Close'].idxmax()

    milestones = [
        (hist.index[0], ipo_price, "IPO", ccy + f"{ipo_price:,.0f}", "#94a3b8", "bottom center"),
        (high_idx, target["high52"], "고점", ccy + f"{target['high52']:,.0f}", "#f59e0b", "top center"),
        (low_idx, target["low52"], "저점", ccy + f"{target['low52']:,.0f}", "#ef4444", "bottom center"),
    ...
    if bt:
        st.markdown("---")
        st.markdown("<div class='section-h'>🔬 백테스트: AI 예측 정확도 검증 <span style='color:#64748b; font-weight:500; font-size:11px; margin-left:8px;'>· 6개월 전 시점 → 현재 비교</span></div>", unsafe_allow_html=True)
        bc1, bc2, bc3, bc4 = st.columns(4)
        with bc1:
            st.markdown(card(f"6개월 전 기준일", bt["past_date"],
        ...
        with bc3:
            st.markdown(card("현재 실제가", f"{ccy}{bt['actual_price']:,.2f}",
                            f"실제: {bt['actual_dir']}"), unsafe_allow_html=True)
        with bc4:
    ...
    # ===== AI 이주의 추천 종목 =====
    st.markdown("---")
    st.markdown("<div class='section-h'>⭐ AI 이주의 추천 종목 <span style='color:#64748b; font-weight:500; font-size:11px; margin-left:8px;'>· S&P500 상위 100개 중 최강 매수 1개 · 주 1회 갱신</span></div>", unsafe_allow_html=True)
    ...
    if pick:
        def mcap_str(m):
            ...
            return f"${m/1e6:.0f}M"
        per_str = f"{pick['per']:.1f}" if pick.get('per') else "N/A"
        st.markdown(f"""<div class='card' style='border:2px solid #22c55e; padding:24px 28px; background:linear-gradient(135deg, rgba(34,197,94,0.08), rgba(34,197,94,0.02));'>
        <div style='display:flex; justify-content:space-between; align-items:start; gap:20px;'>
        <div style='flex:1;'>
        <div style='color:#22c55e; font-size:12px; font-weight:700; letter-spacing:1px; margin-bottom:8px;'>🏆 THIS WEEK'S TOP PICK</div>
        <div style='font-size:32px; font-weight:900; color:#f1f5f9;'>{pick['ticker']}</div>
        <div style='font-size:14px; color:#94a3b8; margin-top:4px;'>{pick['name']}</div>
        </div>
        <div style='text-align:right;'>
        <div style='color:#94a3b8; font-size:11px;'>AI 점수</div>
        <div style='color:#22c55e; font-size:42px; font-weight:900; line-height:1;'>{pick['score']:.0f}</div>
        <div style='color:#64748b; font-size:11px;'>/ 100</div>
        </div>
        </div>
        <div style='display:grid; grid-template-columns:repeat(4, 1fr); gap:12px; margin-top:20px; padding-top:16px; border-top:1px dashed #334155;'>
        <div><div style='color:#94a3b8; font-size:11px;'>현재가</div><div style='color:#f1f5f9; font-weight:800; font-size:16px;'>${pick['price']:,.2f}</div></div>
        <div><div style='color:#94a3b8; font-size:11px;'>시총</div><div style='color:#f1f5f9; font-weight:800; font-size:16px;'>{mcap_str(pick['mcap'])}</div></div>
        <div><div style='color:#94a3b8; font-size:11px;'>1개월 모멘텀</div><div style='color:{"#22c55e" if pick['mom_1m'] >= 0 else "#ef4444"}; font-weight:800; font-size:16px;'>{pick['mom_1m']:+.1f}%</div></div>
        <div><div style='color:#94a3b8; font-size:11px;'>PER</div><div style='color:#f1f5f9; font-weight:800; font-size:16px;'>{per_str}</div></div>
        </div>
        </div>""", unsafe_allow_html=True)
        st.caption(f"💡 위의 종목 검색창에 `{pick['ticker']}` 입력해서 상세 분석 보기 · S&P500 상위 100종목을 매주 자동 스캔하여 점수 1위 선정")
    else:
        st.info("이번 주 추천 가능한 종목이 없습니다. (시장 전체 약세 구간)")
